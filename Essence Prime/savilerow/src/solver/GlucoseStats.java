...
public class GlucoseStats extends Stats
{
    public GlucoseStats(ArrayList<String> stdout_lines) {
        // Pull out # decisions and CPU time.
        for(int i=0; i<stdout_lines.size(); i++) {
            String[] tmp=stdout_lines.get(i).trim().split(" +");
            
            if(tmp.length>=3 && tmp[0].equals("c") && tmp[1].equals("decisions") ) {
                putValue("SolverNodes", tmp[3]);
            }

            if(tmp.length>=5 && tmp[0].equals("c") && (tmp[1].equals("cpu") || tmp[1].equals("CPU")) && tmp[2].equals("time")) {
                putValue("SolverTotalTime", tmp[4]);
            }
